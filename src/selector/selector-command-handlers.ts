  ...
  if (expression) {
    ...
    for (const location of response) {
      if (location["uri"].endsWith(activeEditor.document.fileName)) {
        const startPosition = new Position(location["range"]["start"]["line"], location["range"]["start"]["character"]);
        const endPosition = new Position(location["range"]["end"]["line"], location["range"]["end"]["character"]);
